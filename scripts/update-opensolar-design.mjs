...
// Add image state to an existing generated handler when upgrading older builds.
if (next.includes("const handleOpenSolarDesignLoaded") && !next.includes("setOpenSolarImageUrl(String(payload?.systemImageUrl || \"\"))")) {
  ...
  const replacement = `${handlerMarker}\n    setOpenSolarImageUrl(String(payload?.systemImageUrl || ""))\n`
  if (!next.includes(handlerMarker)) {
    throw new Error("Could not locate the existing OpenSolar design handler")
  }
  next = next.replace(handlerMarker, replacement)
}
...
if (next.includes("const handleOpenSolarDesignLoaded") && !next.includes("const [openSolarImageUrl, setOpenSolarImageUrl]")) {
  ...
  if (!next.includes(marker)) {
    throw new Error("Could not locate the OpenSolar design handler")
  }

  ...
  return (
    <section>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <div style={{ position: "relative", flex: 1, minWidth: 240 }}><Search size={17} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#94a3b8", pointerEvents: "none" }} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search customer, postcode, contract, rep, branch or stage..." aria-label="Search RTS deals" style={{ width: "100%", boxSizing: "border-box", border: "1px solid #d7dee8", borderRadius: 8, padding: "10px 38px 10px 38px", fontSize: 12, outline: "none", background: "#fff" }} />{query && <button type="button" onClick={() => setQuery("")} aria-label="Clear search" style={{ position: "absolute", right: 9, top: "50%", transform: "translateY(-50%)", border: 0, background: "transparent", padding: 4, cursor: "pointer", color: "#64748b", display: "flex" }}><X size={15} /></button>}</div>
        <select value={stage} onChange={(event) => setStage(event.target.value)} style={{ height: 37, minWidth: 190, border: "1px solid #d7dee8", borderRadius: 8, background: "#fff", color: "#334155", padding: "0 10px", fontSize: 11 }}><option value="all">All RTS stages</option>{RTS_STAGES.map((option) => <option key={option} value={option}>{option}</option>)}</select>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto", flex: "0 0 auto" }}><div style={{ padding: "8px 11px", border: "1px solid #dfe5e9", borderRadius: 8, background: "#fff", fontSize: 11, color: "#66737d", whiteSpace: "nowrap" }}><strong style={{ color: "#263645" }}>{filteredDeals.length}</strong> deals</div><div style={{ padding: "8px 11px", border: "1px solid #dfe5e9", borderRadius: 8, background: "#fff", fontSize: 11, color: "#66737d", whiteSpace: "nowrap" }}><strong style={{ color: "#263645" }}>{money(totalValue)}</strong></div></div>
      </div>

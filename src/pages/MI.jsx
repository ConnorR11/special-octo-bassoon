...
function formatRatio(value) { return Number.isFinite(Number(value)) && Number(value) > 0 ? Number(value).toFixed(1) : "—" }
...
export default function MI() {
  ...
  const report = useMemo(() => {
    ...
    const averageConversion = conversionWeeks.length ? conversionWeeks.reduce((total, week) => total + (week.cps_p / week.cps_s), 0) / conversionWeeks.length : null
    const lastConversion = lastWeek.cps_s > 0 ? lastWeek.cps_p / lastWeek.cps_s : null

...
export default function MI() {
  ...
  const report = useMemo(() => {
    ...
    appointments.forEach(appointment => {
      ...
      METRICS.forEach(([field]) => {
        if (field === "net_value") {
          // Net value sold is based only on appointments whose result is Sold.
  ...
  return <section className="mi-page" style={{ maxWidth: 1200, width: "100%", margin: "0 auto", padding: "8px 4px 40px" }}>
    ...
    <div className="mi-table"><div className="mi-table-inner"><div className="mi-table-header"><div>Metric</div><div>Last Week</div><div>Avg Week</div><div>Variance</div><div>Vs Average</div></div>{loading ? <div style={{ padding: 35, textAlign: "center", color: "#89939c", fontSize: 11 }}>Loading management information...</div> : report.rows.map(row => <div className="mi-table-row" key={row.field}><div style={{ fontSize: row.field === "net_value" ? 13 : 14, fontWeight: 800, color: "#263645" }}>{row.label}</div><div style={{ fontSize: 15, fontWeight: 800, color: "#263645" }}>{row.field === "net_value" ? formatCurrency(row.last) : formatNumber(row.last)}</div><div style={{ fontSize: 13, color: "#53616b" }}>{row.field === "net_value" ? formatCurrency(row.average) : formatNumber(row.average)}</div><div style={{ fontSize: 13, fontWeight: 700, color: row.variance >= 0 ? "#21824a" : "#b64b4b" }}>{row.variance >= 0 ? "+" : "-"}{row.field === "net_value" ? formatCurrency(Math.abs(row.variance)) : formatNumber(Math.abs(row.variance))}</div><div style={{ fontSize: 13, fontWeight: 800, color: row.percentage === null ? "#89939c" : row.percentage >= 0 ? "#21824a" : "#b64b4b" }}>{row.percentage === null ? "—" : `${row.percentage >= 0 ? "+" : ""}${row.percentage.toFixed(1)}%`}</div></div>)}</div></div>

...
const METRICS = [["cps_h", "H"], ["cps_c", "C"], ["cps_p", "P"], ["cps_s", "S"], ["net_value", "Net Value Sold"]]
...
export default function MI() {
  ...
  async function loadReport() {
    ...
    try {
      ...
      const { data, error: queryError } = await supabase.from("appointments").select("appointment_row_id, appointment_date, cps_h, cps_c, cps_p, cps_s").gte("appointment_date", `${from}T00:00:00`).lt("appointment_date", `${to}T00:00:00`)
  ...
  return <section className="mi-page" style={{ maxWidth: 1200, width: "100%", margin: "0 auto", padding: "8px 4px 40px" }}>
    ...
    <div className="mi-table"><div className="mi-table-inner"><div className="mi-table-header"><div>Metric</div><div>Last Week</div><div>Avg Week</div><div>Variance</div><div>Vs Average</div></div>{loading ? <div style={{ padding: 35, textAlign: "center", color: "#89939c", fontSize: 11 }}>Loading management information...</div> : report.rows.map(row => <div className="mi-table-row" key={row.field}><div style={{ fontSize: 14, fontWeight: 800, color: "#263645" }}>{row.label}</div><div style={{ fontSize: 15, fontWeight: 800, color: "#263645" }}>{row.field === "net_value" ? formatCurrency(row.last) : formatNumber(row.last)}</div><div style={{ fontSize: 13, color: "#53616b" }}>{row.field === "net_value" ? formatCurrency(row.average) : formatNumber(row.average)}</div><div style={{ fontSize: 13, fontWeight: 700, color: row.variance >= 0 ? "#21824a" : "#b64b4b" }}>{row.variance >= 0 ? "+" : ""}{row.field === "net_value" ? formatCurrency(row.variance) : formatNumber(row.variance)}</div><div style={{ fontSize: 13, fontWeight: 800, color: row.percentage === null ? "#89939c" : row.percentage >= 0 ? "#21824a" : "#b64b4b" }}>{row.percentage === null ? "—" : `${row.percentage >= 0 ? "+" : ""}${row.percentage.toFixed(1)}%`}</div></div>)}</div></div>
    <div className="mi-note">H C P S are counted independently from appointment records. Net Value Sold is summed from the linked deal net_value for appointments in each week.</div>
  </section>
}

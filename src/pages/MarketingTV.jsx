...
// Appointment timestamps are treated as raw database values.
// This deliberately does NOT convert UTC to the browser timezone.
...
function sortBranches(rows) {
  return [
    ...new Set(rows.map((row) => display(row.branch, "Unassigned"))),
  ].sort((a, b) => {
  ...
  return (
    <div className="mtv-summary-wrap">
      <div className="mtv-summary-header">
        <div className="mtv-summary-branch-heading">BRANCH</div>
        <div className="mtv-summary-columns">
          <span>H</span>
          <span>C</span>
          <span>P</span>
          <span>S</span>
          <span className="mtv-summary-value-heading">VALUE</span>
        </div>
      </div>

      <div className="mtv-summary-row mtv-summary-total">
        <span className="mtv-summary-branch">Total</span>
      ...
      {branches.map((branch) => {
        const rows = appointments.filter(
          (row) => display(row.branch, "Unassigned") === branch
        )
  ...
  return (
    ...
    >
      ...
      <td className="mtv-cell mtv-status-cell">
        <StatusTick value={Boolean(appointment.rep_confirmed_time)} />
      </td>

      <td className="mtv-cell mtv-status-cell">
        <StatusTick
          value={Boolean(
            appointment.was_picked_up || appointment.pickup_rep
          )}
        />
      </td>
...
function BranchSection({ branch, appointments, onSelect }) {
  ...
  const sorted = [...appointments].sort((a, b) => {
    const aValue = String(a.appointment_date || "")
    const bValue = String(b.appointment_date || "")
...
export default function MarketingTV({ onSelectAppointment }) {
  ...
  async function loadAppointments(date = selectedDate) {
    ...
    try {
      // Treat the selected calendar date as a UTC calendar date.
      // This prevents the browser's local timezone from changing the query boundary.
  ...
  const visibleAppointments = useMemo(() => {
    if (activeTab === "mastersheet") {
      return appointments.filter((appointment) =>
        isTrueValue(appointment.cps_c)
      )
    }

    if (activeTab === "handover") {
      return appointments.filter((appointment) =>
        isNotTrue(appointment.cps_c)
      )
    }
  ...
  return (
    <section className="marketing-tv-page">
      <style>{`
        ...
        .mtv-summary-branch-heading { text-align: left; }

...
const columns="1.55fr 1.15fr .95fr .9fr 1fr 1fr .95fr 30px"
function DealRow({deal,setSelected}) {
  const customer=deal?.customer_name||deal?.name||"Unnamed customer", commissionDate=getCommissionDate(deal), surveyCosting=getSurveyCosting(deal), commission=getCommission(deal)
  return <button type="button" onClick={()=>setSelected?.(deal)} style={{width:"100%",display:"grid",gridTemplateColumns:columns,gap:12,alignItems:"center",padding:"13px 14px",border:0,borderBottom:"1px solid #eef1f3",background:"#fff",textAlign:"left",cursor:"pointer",fontFamily:"inherit"}}>
    <div style={{display:"flex",alignItems:"center",gap:9,minWidth:0}}><div style={{width:30,height:30,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:"#e8f4fd",color:"#1676b8",fontSize:9,fontWeight:800}}>{getInitials(customer)}</div><div style={{minWidth:0}}><div style={{fontSize:11,fontWeight:700,color:"#263645",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{customer}</div><div style={{marginTop:3,fontSize:9,color:"#8a959d",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{deal?.contract_number||deal?.product||"No contract number"}</div></div></div>
    ...
    <div style={{fontSize:10,color:surveyCosting===null?"#a0a8ae":"#263645",fontWeight:surveyCosting===null?400:700}}>{surveyCosting===null?"—":money(surveyCosting)}</div>
    ...
    <div style={{display:"flex",alignItems:"center",gap:5,color:"#66737d",fontSize:10}}><MapPin size={13}/><span>{getBranchName(deal)}</span></div><div style={{display:"flex",justifyContent:"flex-end",color:"#9aa5ad"}}><ChevronRight size={17}/></div>
  </button>
}
export default function SalesCommission({deals=[],loading=false,setSelected}) {
  const [query,setQuery]=useState(""),[rep,setRep]=useState("all"),[branch,setBranch]=useState("all"),[commissionDate,setCommissionDate]=useState("all")
  ...
  const commissionDates=useMemo(()=>Array.from(new Set(deals.map(getCommissionDate).filter(Boolean))).sort((a,b)=>new Date(a)-new Date(b)),[deals])
  const filtered=useMemo(()=>{
    const search=query.trim().toLowerCase()
    return [...deals].filter(deal=>{
      const date=getCommissionDate(deal)
      ...
      const ad=getCommissionDate(a),bd=getCommissionDate(b); if(!ad&&!bd)return getBranchName(a).localeCompare(getBranchName(b)); if(!ad)return 1; if(!bd)return -1
      const dateComparison=new Date(ad)-new Date(bd); if(dateComparison)return dateComparison
      const branchComparison=getBranchName(a).localeCompare(getBranchName(b)); if(branchComparison)return branchComparison
      return (a?.customer_name||a?.name||"Unnamed customer").localeCompare(b?.customer_name||b?.name||"Unnamed customer")
    })
  },[deals,query,rep,branch,commissionDate])
  const totalCommission=filtered.reduce((total,deal)=>total+(getCommission(deal)??0),0),totalNetSalesValue=filtered.reduce((total,deal)=>total+getNetSalesValue(deal),0)
  return <section>
    ...
    {loading?<div style={{padding:35,textAlign:"center",color:"#89939c",fontSize:11,border:"1px solid #e0e5e9",borderRadius:10,background:"#fff"}}>Loading commission data...</div>:!filtered.length?<div style={{padding:40,textAlign:"center",border:"1px solid #e0e5e9",borderRadius:10,background:"#fff",color:"#89939c",fontSize:11}}>No commission records found.</div>:<div style={{border:"1px solid #e0e5e9",borderRadius:10,background:"#fff",overflow:"hidden"}}><div style={{display:"grid",gridTemplateColumns:columns,gap:12,alignItems:"center",padding:"10px 14px",background:"#f3f6f8",borderBottom:"1px solid #e0e5e9",color:"#687782",fontSize:9,fontWeight:800,textTransform:"uppercase"}}><div>Customer</div><div>Sales Rep</div><div>Commission Date</div><div>Survey Costing</div><div>Net Sales Value</div><div>Estimated Commission Due</div><div>Branch</div><div/></div>{filtered.map(deal=><DealRow key={deal?.id||deal?.deal_id} deal={deal} setSelected={setSelected}/>)}</div>}
  </section>
}

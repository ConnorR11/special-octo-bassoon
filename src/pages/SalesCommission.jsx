...
function DealRow({deal,setSelected}) {
  const customer=deal?.customer_name||deal?.name||"Unnamed customer", commissionDate=getCommissionDate(deal), commission=getCommission(deal)
...
export default function SalesCommission({deals=[],loading=false,setSelected}) {
  ...
  return <section>
    ...
    {loading?<div style={{padding:35,textAlign:"center",color:"#89939c",fontSize:11,border:"1px solid #e0e5e9",borderRadius:10,background:"#fff"}}>Loading commission data...</div>:!filtered.length?<div style={{padding:40,textAlign:"center",border:"1px solid #e0e5e9",borderRadius:10,background:"#fff",color:"#89939c",fontSize:11}}>No commission records found.</div>:<div style={{border:"1px solid #e0e5e9",borderRadius:10,background:"#fff",overflow:"hidden"}}><div style={{display:"grid",gridTemplateColumns:columns,gap:12,alignItems:"center",padding:"10px 14px",background:"#f3f6f8",borderBottom:"1px solid #e0e5e9",color:"#687782",fontSize:9,fontWeight:800,textTransform:"uppercase"}}><div>Customer</div><div>Sales Rep</div><div>Commission Date</div><div>Net Sales Value</div><div>Estimated Commission Due</div><div>Branch</div><div/></div>{filtered.map(deal=><DealRow key={deal?.id||deal?.deal_id} deal={deal} setSelected={setSelected}/>)}</div>}
  </section>
}

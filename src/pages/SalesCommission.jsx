...
export default function SalesCommission({ deals=[], loading=false, setSelected }) {
  ...
  return <section>
    <div style={{display:"flex",alignItems:"stretch",gap:10,marginBottom:16}}>
      <div style={{position:"relative",flex:"1 1 360px",minWidth:180}}>
        <Search size={17} style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#94a3b8"}}/>
        <input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search customer, contract, rep or branch..." style={{width:"100%",height:46,boxSizing:"border-box",border:"1px solid #d7dee8",borderRadius:9,padding:"10px 38px",fontSize:12,outline:"none"}}/>
        {query&&<button type="button" onClick={()=>setQuery("")} style={{position:"absolute",right:9,top:"50%",transform:"translateY(-50%)",border:0,background:"transparent",cursor:"pointer",color:"#64748b"}}><X size={15}/></button>}
      </div>
      ...
      <div style={{display:"flex",gap:8,flex:"0 0 auto"}}>
        <div style={{minWidth:95,padding:"6px 14px",border:"1px solid #d9e2e8",borderRadius:9,background:"#fff",display:"flex",flexDirection:"column",justifyContent:"center"}}><div style={{fontSize:9,fontWeight:700,color:"#7b8790",textTransform:"uppercase"}}>Deals</div><div style={{fontSize:20,fontWeight:800,color:"#263645"}}>{filtered.length}</div></div>
        <div style={{minWidth:135,padding:"6px 14px",border:"1px solid #cfe2ef",borderRadius:9,background:"#f7fbff",display:"flex",flexDirection:"column",justifyContent:"center"}}><div style={{fontSize:9,fontWeight:700,color:"#6d8495",textTransform:"uppercase"}}>Net Sales</div><div style={{fontSize:20,fontWeight:800,color:"#263645"}}>{money(totalNetSalesValue)}</div></div>
        <div style={{minWidth:155,padding:"6px 14px",border:"1px solid #cfe2ef",borderRadius:9,background:"#f7fbff",display:"flex",flexDirection:"column",justifyContent:"center"}}><div style={{fontSize:9,fontWeight:700,color:"#6d8495",textTransform:"uppercase"}}>Estimated Commission</div><div style={{fontSize:20,fontWeight:800,color:"#1676b8"}}>{money(totalCommission)}</div></div>
      </div>
    </div>
    ...
    {loading?<div style={{padding:35,textAlign:"center",color:"#89939c",fontSize:11,border:"1px solid #e0e5e9",borderRadius:10,background:"#fff"}}>Loading commission data...</div>:!filtered.length?<div style={{padding:40,textAlign:"center",border:"1px solid #e0e5e9",borderRadius:10,background:"#fff",color:"#89939c",fontSize:11}}>No commission records found.</div>:<div style={{border:"1px solid #e0e5e9",borderRadius:10,background:"#fff",overflow:"hidden"}}>
      <div style={{display:"grid",gridTemplateColumns:columns,gap:12,alignItems:"center",padding:"10px 14px",background:"#f3f6f8",borderBottom:"1px solid #e0e5e9",color:"#687782",fontSize:9,fontWeight:800,textTransform:"uppercase"}}><div>Customer</div><div>Sales Rep</div><div>Commission Date</div><div>Net Sales Value</div><div>Commission</div><div>Branch</div><div/></div>

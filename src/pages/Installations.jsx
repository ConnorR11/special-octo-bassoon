...
export default function Installations({ setSelected }) {
  ...
  const groupedStages = useMemo(() => {
    ...
    const ungroupedStages = Array.from(stageMap.entries())
      .filter(([stage]) => !GROUPS[0].stages.includes(stage))
  ...
  function DealCard({ deal }) {
    return (
      <button type="button" onClick={() => openDeal(deal)} style={{ width: "100%", padding: 12, border: "1px solid #e0e5e9", borderRadius: 8, background: "#fff", textAlign: "left", cursor: "pointer", fontFamily: "inherit", boxShadow: "0 1px 2px rgba(0,0,0,.04)" }} onMouseEnter={(event) => { event.currentTarget.style.borderColor = "#b9c8d3"; event.currentTarget.style.boxShadow = "0 3px 10px rgba(0,0,0,.07)" }} onMouseLeave={(event) => { event.currentTarget.style.borderColor = "#e0e5e9"; event.currentTarget.style.boxShadow = "0 1px 2px rgba(0,0,0,.04)" }}>
  ...
  function StageColumn({ column }) {
    return (
      <div style={{ flex: "0 0 280px", width: 280, minHeight: 170, background: "#f5f7f9", border: "1px solid #e1e6ea", borderRadius: 8, overflow: "hidden" }}>
        <div style={{ padding: "10px 12px", borderBottom: "1px solid #e1e6ea", background: "#fff", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: "#263645", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={column.stage}>{column.stage}</div>
          <span style={{ minWidth: 22, height: 20, padding: "0 6px", display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 10, background: "#eef2f5", color: "#5d6a75", fontSize: 9, fontWeight: 800 }}>{column.items.length}</span>
        </div>
        <div style={{ padding: 9, display: "grid", gap: 9 }}>
          {column.items.map((deal) => <DealCard key={getDealId(deal)} deal={deal} />)}
          {column.items.length === 0 && <div style={{ padding: "20px 8px", textAlign: "center", color: "#9aa5ad", fontSize: 10 }}>No deals</div>}
        </div>
      </div>
    )
  }
  ...
  function KanbanGroup({ group }) {
    ...
    return (
      <div style={{ flex: collapsed ? "0 0 52px" : "0 0 auto", minWidth: collapsed ? 52 : 0, border: `1px solid ${group.border}`, borderRadius: 10, overflow: "hidden", background: "#fff", transition: "flex-basis .15s ease" }}>
        <button type="button" onClick={() => toggleGroup(group.key)} aria-expanded={!collapsed} style={{ width: collapsed ? 52 : "100%", minHeight: 48, display: "flex", alignItems: "center", gap: 8, padding: collapsed ? "10px 0" : "0 14px", justifyContent: collapsed ? "center" : "flex-start", border: 0, borderBottom: collapsed ? 0 : `1px solid ${group.border}`, background: group.background, color: group.text, cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}>
          <ChevronDown size={17} style={{ flex: "0 0 auto", transform: collapsed ? "rotate(-90deg)" : "none", transition: "transform .15s ease" }} />
          {!collapsed && <span style={{ fontSize: 13, fontWeight: 800, flex: 1 }}>{group.label}</span>}
          {!collapsed && <span style={{ minWidth: 28, height: 24, padding: "0 7px", display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 12, background: group.badge, color: group.text, fontSize: 10, fontWeight: 800 }}>{group.count}</span>}
        </button>
    ...
    <section>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 18 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: 22, color: "#222" }}>Kanban</h1><p style={{ margin: "5px 0 0", fontSize: 11, color: "#888" }}>Read-only installation board grouped by Pipedrive stage</p></div>
        </div>

...
export default function SalesKPI() {
  ...
  function selectPeriod(value) {
    ...
    setPeriod(value); setStartDate(dates.start); setEndDate(dates.end)
  }

  async function loadKPI() {
    if (!supabase) { setError("Supabase is not configured."); return }
    if (!startDate || !endDate) { setError("Please enter both a start and end date."); return }
    if (startDate > endDate) { setError("The start date cannot be after the end date."); return }
    ...
    try {
      ...
      const [appointmentsResult, profilesResult] = await Promise.all([
        supabase.from("appointments").select("appointment_row_id, rep_allocated, cps_h, cps_c, cps_p, cps_s, job_type, branch").gte("appointment_date", `${startDate}T00:00:00`).lt("appointment_date", endExclusiveString).order("appointment_date", { ascending: true }),
  ...
  const filteredAppointments = useMemo(() => appointments.filter(a => {
    return (jobTypeFilter === "all" || (a.job_type || "Unspecified") === jobTypeFilter) && (branchFilter === "all" || (a.branch || "Unspecified") === branchFilter)
  ...
  return <>
    <style>{`
      .sales-kpi-page{min-height:100%;padding:28px 32px 40px;background:#f5f7fa;color:#0f172a;box-sizing:border-box}.sales-kpi-container{max-width:1400px;margin:0 auto}.sales-kpi-header{display:flex;justify-content:space-between;align-items:flex-end;gap:28px;margin-bottom:16px}.sales-kpi-periods{display:flex;gap:7px;margin-bottom:0}.sales-kpi-period{height:38px;padding:0 15px;border:1px solid #d7dee7;border-radius:8px;background:#fff;color:#475569;font-weight:700;font-size:12px;cursor:pointer}.sales-kpi-period:hover{border-color:#0877bd}.sales-kpi-period.active{background:#0877bd;border-color:#0877bd;color:#fff}.sales-kpi-date-card{display:flex;align-items:flex-end;gap:12px;padding:14px;background:#fff;border:1px solid #e2e8f0;border-radius:12px;box-shadow:0 2px 8px rgba(15,23,42,.04)}.sales-kpi-date-field{display:flex;flex-direction:column;gap:6px}.sales-kpi-date-field span{font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.06em}.sales-kpi-date-input{display:flex;align-items:center;gap:8px;height:38px;padding:0 10px;border:1px solid #d7dee7;border-radius:8px;background:#fff;box-sizing:border-box}.sales-kpi-date-input svg{color:#64748b;flex:0 0 auto}.sales-kpi-date-input input{border:0;outline:0;background:transparent;color:#0f172a;font:inherit;font-size:13px;min-width:125px}.sales-kpi-run{height:38px;padding:0 15px;border:0;border-radius:8px;background:#0877bd;color:#fff;font-weight:700;font-size:13px;cursor:pointer;display:inline-flex;align-items:center;gap:7px}.sales-kpi-run:hover{background:#06659f}.sales-kpi-run:disabled{opacity:.65;cursor:default}.sales-kpi-summary{display:flex;align-items:center;gap:8px;margin:0 0 16px;color:#64748b;font-size:12px;font-weight:600}.sales-kpi-summary-label{color:#0f172a;font-weight:750}.sales-kpi-panel{background:#fff;border:1px solid #e2e8f0;border-radius:14px;box-shadow:0 2px 10px rgba(15,23,42,.05);overflow:hidden}.sales-kpi-panel-header{padding:20px 22px;border-bottom:1px solid #e8edf2;display:flex;justify-content:space-between;align-items:center;gap:20px}.sales-kpi-panel-title{display:flex;align-items:center;gap:11px}.sales-kpi-panel-icon{width:34px;height:34px;border-radius:9px;display:grid;place-items:center;background:#eaf5fc;color:#0877bd}.sales-kpi-panel-header h2{margin:0;font-size:18px;font-weight:750}.sales-kpi-panel-header p{margin:3px 0 0;color:#64748b;font-size:12px}.sales-kpi-date-range{color:#64748b;font-size:12px;font-weight:600;white-space:nowrap}.sales-kpi-filter-bar{display:flex;align-items:center;gap:10px;padding:14px 22px;border-bottom:1px solid #e8edf2;background:#fff}.sales-kpi-filter-label{color:#64748b;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;margin-right:2px}.sales-kpi-filter-select{height:34px;min-width:150px;padding:0 10px;border:1px solid #d7dee7;border-radius:8px;background:#fff;color:#0f172a;font-size:12px;outline:none;cursor:pointer}.sales-kpi-filter-clear{height:34px;padding:0 11px;border:1px solid #d7dee7;border-radius:8px;background:#fff;color:#64748b;font-size:12px;font-weight:700;cursor:pointer}.sales-kpi-table-wrap{overflow-x:auto}.sales-kpi-table{width:100%;border-collapse:collapse;font-size:13px}.sales-kpi-table th{height:46px;padding:0 22px;text-align:left;background:#f8fafc;color:#64748b;font-size:11px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;border-bottom:1px solid #e2e8f0;white-space:nowrap;user-select:none}.sales-kpi-table th:not(:first-child),.sales-kpi-table td:not(:first-child){text-align:right}.sales-kpi-table th.sortable{cursor:pointer}.sales-kpi-table th.sortable:hover{color:#0877bd}.sales-kpi-sort{display:inline-flex;align-items:center;gap:4px}.sales-kpi-table td{height:48px;padding:0 22px;border-bottom:1px solid #edf1f5;color:#1e293b;font-variant-numeric:tabular-nums}.sales-kpi-table tbody tr:hover{background:#f8fbfd}.sales-kpi-rep{display:flex;align-items:center;gap:9px;text-align:left;font-weight:650}.sales-kpi-rep-dot{width:8px;height:8px;border-radius:50%;background:#0877bd;flex:0 0 auto}.sales-kpi-total td{height:52px;background:#eef5f9;border-bottom:2px solid #d7e4ed;font-weight:800;color:#0f172a}.sales-kpi-total td:not(:first-child){font-size:14px}.sales-kpi-empty{text-align:center!important;color:#64748b!important;padding:36px 20px!important;height:auto!important}.sales-kpi-error{margin-bottom:18px;padding:12px 15px;border-radius:10px;background:#fef2f2;border:1px solid #fecaca;color:#991b1b;font-size:13px}.sales-kpi-spin{animation:sales-kpi-spin 1s linear infinite}@keyframes sales-kpi-spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}@media(max-width:900px){.sales-kpi-header{flex-direction:column;align-items:stretch}.sales-kpi-periods{margin-bottom:10px}.sales-kpi-date-card{width:fit-content;max-width:100%;flex-wrap:wrap}}@media(max-width:600px){.sales-kpi-periods{flex-wrap:wrap}.sales-kpi-page{padding:20px 14px 30px}.sales-kpi-date-card{width:100%;box-sizing:border-box}.sales-kpi-date-field{flex:1;min-width:135px}.sales-kpi-date-input input{min-width:0;width:100%}.sales-kpi-run{flex:1;justify-content:center}.sales-kpi-panel-header{align-items:flex-start;flex-direction:column}.sales-kpi-date-range{white-space:normal}}
    `}</style>
    <div className="sales-kpi-page"><div className="sales-kpi-container">
      <header className="sales-kpi-header">
        <div className="sales-kpi-periods">
          <button type="button" className={`sales-kpi-period ${period === "last" ? "active" : ""}`} onClick={() => selectPeriod("last")}>Last Week</button>
          <button type="button" className={`sales-kpi-period ${period === "this" ? "active" : ""}`} onClick={() => selectPeriod("this")}>This Week</button>
          <button type="button" className={`sales-kpi-period ${period === "ytd" ? "active" : ""}`} onClick={() => selectPeriod("ytd")}>Year to Date</button>
        </div>
        <div className="sales-kpi-date-card">
          <label className="sales-kpi-date-field"><span>Start date</span><div className="sales-kpi-date-input"><CalendarDays size={15}/><input type="date" value={startDate} onChange={e => { setStartDate(e.target.value); setPeriod("custom") }}/></div></label>
          <label className="sales-kpi-date-field"><span>End date</span><div className="sales-kpi-date-input"><CalendarDays size={15}/><input type="date" value={endDate} onChange={e => { setEndDate(e.target.value); setPeriod("custom") }}/></div></label>
          <button type="button" className="sales-kpi-run" onClick={loadKPI} disabled={loading}><RefreshCw size={14} className={loading ? "sales-kpi-spin" : ""}/>{loading ? "Loading" : "Run"}</button>
        </div>
      </header>
      ...
      <section className="sales-kpi-panel">
        ...
        <div className="sales-kpi-filter-bar"><span className="sales-kpi-filter-label">Filter</span><select className="sales-kpi-filter-select" value={jobTypeFilter} onChange={e => setJobTypeFilter(e.target.value)}><option value="all">All job types</option>{jobTypes.map(j => <option key={j} value={j}>{j}</option>)}</select><select className="sales-kpi-filter-select" value={branchFilter} onChange={e => setBranchFilter(e.target.value)}><option value="all">All branches</option>{branches.map(b => <option key={b} value={b}>{b}</option>)}</select>{(jobTypeFilter !== "all" || branchFilter !== "all") && <button type="button" className="sales-kpi-filter-clear" onClick={() => { setJobTypeFilter("all"); setBranchFilter("all") }}>Clear filters</button>}</div>
        <div className="sales-kpi-table-wrap"><table className="sales-kpi-table"><thead><tr>{[["rep_allocated","REP"],["h","H"],["c","C"],["p","P"],["s","S"],["net_value","NET VALUE"],["conversion","CONVERSION"],["bo_percent","BO%"],["avg_order_value","AVG"]].map(([field,label]) => <th key={field} className="sortable" onClick={() => changeSort(field)}><span className="sales-kpi-sort">{label}<SortIcon field={field}/></span></th>)}</tr></thead><tbody>
          <tr className="sales-kpi-total"><td>Total</td><td>{formatNumber(totals.h)}</td><td>{formatNumber(totals.c)}</td><td>{formatNumber(totals.p)}</td><td>{formatNumber(totals.s)}</td><td>{formatCurrency(totals.net_value)}</td><td>{formatConversion(totalConversion)}</td><td>{formatPercent(totalBoPercent)}</td><td>{formatCurrency(totalAvgOrderValue)}</td></tr>

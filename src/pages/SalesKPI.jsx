...
export default function SalesKPI() {
  ...
  async function loadKPI() {
    ...
    const { data, error: supabaseError } = await supabase
      .from("appointments")
      .select("rep_allocated, cps_h, cps_c, cps_p, cps_s, job_type, branch, deals(net_value)")
    ...
    if (supabaseError) {
      ...
      setError(supabaseError.message); setRows([]); setLoading(false); return
    }

    setAppointments(data || [])
    setJobTypeFilter("all"); setBranchFilter("all"); setLoading(false)
  }
  ...
  useEffect(() => {
    ...
    filteredAppointments.forEach((appointment) => {
      ...
      if (!grouped.has(key)) grouped.set(key, { key, rep_allocated: appointment.rep_allocated || "Unallocated", h: 0, c: 0, p: 0, s: 0, net_value: 0 })
  ...
  const jobTypes = useMemo(() => Array.from(new Set(appointments.map((appointment) => appointment.job_type || "Unspecified"))).sort((a, b) => a.localeCompare(b)), [appointments])
  const branches = useMemo(() => Array.from(new Set(appointments.map((appointment) => appointment.branch || "Unspecified"))).sort((a, b) => a.localeCompare(b)), [appointments])
  ...
  const totals = useMemo(() => rows.reduce((total, row) => ({ h: total.h + row.h, c: total.c + row.c, p: total.p + row.p, s: total.s + row.s, net_value: total.net_value + row.net_value }), { h: 0, c: 0, p: 0, s: 0, net_value: 0 }), [rows])
    ...
    <>
      ...
      <div className="sales-kpi-page"><div className="sales-kpi-container">
        <header className="sales-kpi-header"><div><div className="sales-kpi-eyebrow">Sales</div><h1>Sales KPI</h1><p>H, C, P, S performance and net sales value by rep.</p></div>
          <div className="sales-kpi-date-card"><label className="sales-kpi-date-field"><span>Start date</span><div className="sales-kpi-date-input"><CalendarDays size={15}/><input type="date" value={startDate} onChange={(event)=>setStartDate(event.target.value)}/></div></label><label className="sales-kpi-date-field"><span>End date</span><div className="sales-kpi-date-input"><CalendarDays size={15}/><input type="date" value={endDate} onChange={(event)=>setEndDate(event.target.value)}/></div></label><button className="sales-kpi-run" onClick={loadKPI} disabled={loading}><RefreshCw size={14} className={loading ? "sales-kpi-spin" : ""}/>{loading ? "Loading" : "Run"}</button></div>
        </header>
        ...
        <section className="sales-kpi-panel"><div className="sales-kpi-panel-header"><div className="sales-kpi-panel-title"><div className="sales-kpi-panel-icon"><Users size={17}/></div><div><h2>Sales Rep Performance</h2><p>Appointments grouped by allocated sales rep.</p></div></div><div className="sales-kpi-date-range">{formatDate(startDate)} – {formatDate(endDate)}</div></div>
          <div className="sales-kpi-filter-bar"><span className="sales-kpi-filter-label">Filter</span><select className="sales-kpi-filter-select" value={jobTypeFilter} onChange={(event)=>setJobTypeFilter(event.target.value)}><option value="all">All job types</option>{jobTypes.map((jobType)=><option key={jobType} value={jobType}>{jobType}</option>)}</select><select className="sales-kpi-filter-select" value={branchFilter} onChange={(event)=>setBranchFilter(event.target.value)}><option value="all">All branches</option>{branches.map((branch)=><option key={branch} value={branch}>{branch}</option>)}</select>{(jobTypeFilter !== "all" || branchFilter !== "all") && <button className="sales-kpi-filter-clear" onClick={()=>{setJobTypeFilter("all");setBranchFilter("all")}}>Clear filters</button>}</div>
          <div className="sales-kpi-table-wrap"><table className="sales-kpi-table"><thead><tr><th className="sortable" onClick={()=>changeSort("rep_allocated")}><span className="sales-kpi-sort">REP <SortIcon field="rep_allocated"/></span></th><th className="sortable" onClick={()=>changeSort("h")}><span className="sales-kpi-sort">H <SortIcon field="h"/></span></th><th className="sortable" onClick={()=>changeSort("c")}><span className="sales-kpi-sort">C <SortIcon field="c"/></span></th><th className="sortable" onClick={()=>changeSort("p")}><span className="sales-kpi-sort">P <SortIcon field="p"/></span></th><th className="sortable" onClick={()=>changeSort("s")}><span className="sales-kpi-sort">S <SortIcon field="s"/></span></th><th className="sortable" onClick={()=>changeSort("net_value")}><span className="sales-kpi-sort">NET VALUE <SortIcon field="net_value"/></span></th></tr></thead><tbody>
            <tr className="sales-kpi-total"><td><div className="sales-kpi-rep"><span className="sales-kpi-rep-dot"/>Total</div></td><td>{formatNumber(totals.h)}</td><td>{formatNumber(totals.c)}</td><td>{formatNumber(totals.p)}</td><td>{formatNumber(totals.s)}</td><td>{formatCurrency(totals.net_value)}</td></tr>
            {loading ? <tr><td colSpan="6" className="sales-kpi-empty">Loading sales KPI...</td></tr> : sortedRows.length === 0 ? <tr><td colSpan="6" className="sales-kpi-empty">No appointments found for the selected filters.</td></tr> : sortedRows.map((row)=><tr key={row.key} className={row.key === "__unallocated__" ? "unallocated" : ""}><td><div className="sales-kpi-rep"><span className="sales-kpi-rep-dot"/>{row.rep_allocated}</div></td><td>{formatNumber(row.h)}</td><td>{formatNumber(row.c)}</td><td>{formatNumber(row.p)}</td><td>{formatNumber(row.s)}</td><td>{formatCurrency(row.net_value)}</td></tr>)}
          </tbody></table></div>
        </section>
      </div></div>
    </>

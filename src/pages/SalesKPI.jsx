...
export default function SalesKPI() {
  ...
  async function loadKPI() {
    ...
    try {
      ...
      const appointmentIds = loadedAppointments.map(a => a.appointment_row_id).filter(Boolean)
      ...
      for (let i = 0; i < appointmentIds.length; i += DEAL_BATCH_SIZE) {
        ...
        const { data: dealData, error: dealsError } = await supabase
          .from("deals").select("appointment_row_id, net_value").in("appointment_row_id", batch)
  ...
  const filteredAppointments = useMemo(() => appointments.filter(appointment => {
    const jobTypeMatches = jobTypeFilter === "all" || (appointment.job_type || "Unspecified") === jobTypeFilter
    const branchMatches = branchFilter === "all" || (appointment.branch || "Unspecified") === branchFilter
  ...
  function changeSort(field) {
    if (sortField === field) { setSortDirection(current => current === "desc" ? "asc" : "desc"); return }
    setSortField(field); setSortDirection(field === "rep_allocated" ? "asc" : "desc")
  }
  ...
  return <>
    <style>{`
      .sales-kpi-page{min-height:100%;padding:28px 32px 40px;background:#f5f7fa;color:#0f172a;box-sizing:border-box}.sales-kpi-container{max-width:1400px;margin:0 auto}.sales-kpi-header{display:flex;justify-content:space-between;align-items:flex-end;gap:28px;margin-bottom:16px}.sales-kpi-eyebrow{color:#0877bd;font-size:12px;font-weight:800;letter-spacing:.09em;text-transform:uppercase;margin-bottom:6px}.sales-kpi-header h1{margin:0;font-size:32px;line-height:1.1;font-weight:750;letter-spacing:-.025em}.sales-kpi-header p{margin:8px 0 0;color:#64748b;font-size:14px}.sales-kpi-date-card{display:flex;align-items:flex-end;gap:12px;padding:14px;background:#fff;border:1px solid #e2e8f0;border-radius:12px;box-shadow:0 2px 8px rgba(15,23,42,.04)}.sales-kpi-date-field{display:flex;flex-direction:column;gap:6px}.sales-kpi-date-field span{font-size:11px;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.06em}.sales-kpi-date-input{display:flex;align-items:center;gap:8px;height:38px;padding:0 10px;border:1px solid #d7dee7;border-radius:8px;background:#fff;box-sizing:border-box}.sales-kpi-date-input svg{color:#64748b;flex:0 0 auto}.sales-kpi-date-input input{border:0;outline:0;background:transparent;color:#0f172a;font:inherit;font-size:13px;min-width:125px}.sales-kpi-run{height:38px;padding:0 15px;border:0;border-radius:8px;background:#0877bd;color:#fff;font-weight:700;font-size:13px;cursor:pointer;display:inline-flex;align-items:center;gap:7px}.sales-kpi-run:hover{background:#06659f}.sales-kpi-run:disabled{opacity:.65;cursor:default}.sales-kpi-summary{display:flex;align-items:center;gap:8px;margin:0 0 16px;color:#64748b;font-size:12px;font-weight:600}.sales-kpi-summary-label{color:#0f172a;font-weight:750}.sales-kpi-panel{background:#fff;border:1px solid #e2e8f0;border-radius:14px;box-shadow:0 2px 10px rgba(15,23,42,.05);overflow:hidden}.sales-kpi-panel-header{padding:20px 22px;border-bottom:1px solid #e8edf2;display:flex;justify-content:space-between;align-items:center;gap:20px}.sales-kpi-panel-title{display:flex;align-items:center;gap:11px}.sales-kpi-panel-icon{width:34px;height:34px;border-radius:9px;display:grid;place-items:center;background:#eaf5fc;color:#0877bd}.sales-kpi-panel-header h2{margin:0;font-size:18px;font-weight:750}.sales-kpi-panel-header p{margin:3px 0 0;color:#64748b;font-size:12px}.sales-kpi-date-range{color:#64748b;font-size:12px;font-weight:600;white-space:nowrap}.sales-kpi-filter-bar{display:flex;align-items:center;gap:10px;padding:14px 22px;border-bottom:1px solid #e8edf2;background:#fff}.sales-kpi-filter-label{color:#64748b;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;margin-right:2px}.sales-kpi-filter-select{height:34px;min-width:150px;padding:0 30px 0 10px;border:1px solid #d7dee7;border-radius:8px;background:#fff;color:#0f172a;font:inherit;font-size:12px;outline:none;cursor:pointer}.sales-kpi-filter-select:focus{border-color:#0877bd;box-shadow:0 0 0 3px rgba(8,119,189,.10)}.sales-kpi-filter-clear{height:34px;padding:0 11px;border:1px solid #d7dee7;border-radius:8px;background:#fff;color:#64748b;font-size:12px;font-weight:700;cursor:pointer}.sales-kpi-table-wrap{overflow-x:auto}.sales-kpi-table{width:100%;border-collapse:collapse;font-size:13px}.sales-kpi-table th{height:46px;padding:0 22px;text-align:left;background:#f8fafc;color:#64748b;font-size:11px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;border-bottom:1px solid #e2e8f0;white-space:nowrap;user-select:none}.sales-kpi-table th:not(:first-child),.sales-kpi-table td:not(:first-child){text-align:right}.sales-kpi-table th.sortable{cursor:pointer}.sales-kpi-table th.sortable:hover{color:#0877bd}.sales-kpi-sort{display:inline-flex;align-items:center;gap:4px}.sales-kpi-table td{height:48px;padding:0 22px;border-bottom:1px solid #edf1f5;color:#1e293b;font-variant-numeric:tabular-nums}.sales-kpi-table tbody tr:hover{background:#f8fbfd}.sales-kpi-table tbody tr.unallocated{background:#fffaf0}.sales-kpi-rep{display:flex;align-items:center;gap:9px;text-align:left;font-weight:650}.sales-kpi-rep-dot{width:8px;height:8px;border-radius:50%;background:#0877bd;flex:0 0 auto}.sales-kpi-table tr.unallocated .sales-kpi-rep-dot{background:#f59e0b}.sales-kpi-total td{height:52px;background:#eef5f9;border-bottom:2px solid #d7e4ed;font-weight:800;color:#0f172a}.sales-kpi-total td:not(:first-child){font-size:14px}.sales-kpi-total .sales-kpi-rep-dot{background:#0f172a}.sales-kpi-empty{text-align:center!important;color:#64748b!important;padding:36px 20px!important;height:auto!important}.sales-kpi-error{margin-bottom:18px;padding:12px 15px;border-radius:10px;background:#fef2f2;border:1px solid #fecaca;color:#991b1b;font-size:13px}.sales-kpi-error strong{display:block;margin-bottom:2px}.sales-kpi-spin{animation:sales-kpi-spin 1s linear infinite}@keyframes sales-kpi-spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}@media(max-width:900px){.sales-kpi-header{flex-direction:column;align-items:stretch}.sales-kpi-date-card{width:fit-content;max-width:100%;flex-wrap:wrap}}@media(max-width:600px){.sales-kpi-filter-bar{flex-wrap:wrap;padding:12px 14px}.sales-kpi-filter-select{flex:1;min-width:130px}.sales-kpi-page{padding:20px 14px 30px}.sales-kpi-header h1{font-size:27px}.sales-kpi-date-card{width:100%;box-sizing:border-box}.sales-kpi-date-field{flex:1;min-width:135px}.sales-kpi-date-input input{min-width:0;width:100%}.sales-kpi-run{flex:1;justify-content:center}.sales-kpi-panel-header{align-items:flex-start;flex-direction:column}.sales-kpi-date-range{white-space:normal}}
    `}</style>
    <div className="sales-kpi-page"><div className="sales-kpi-container">
      <header className="sales-kpi-header"><div><div className="sales-kpi-eyebrow">Sales</div><h1>Sales KPI</h1><p>H, C, P, S performance and net sales value by rep.</p></div>
        <div className="sales-kpi-date-card"><label className="sales-kpi-date-field"><span>Start date</span><div className="sales-kpi-date-input"><CalendarDays size={15}/><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></div></label><label className="sales-kpi-date-field"><span>End date</span><div className="sales-kpi-date-input"><CalendarDays size={15}/><input type="date" value={endDate} onChange={e=>setEndDate(e.target.value)}/></div></label><button className="sales-kpi-run" onClick={loadKPI} disabled={loading}><RefreshCw size={14} className={loading?"sales-kpi-spin":""}/>{loading?"Loading":"Run"}</button></div>
      </header>

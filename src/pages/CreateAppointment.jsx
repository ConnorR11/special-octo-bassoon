...
export default function CreateAppointment({ onBack, onCreated }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const appointment = {
        ...Object.fromEntries(
          Object.entries(form).map(([key, value]) => [key, value.trim ? value.trim() : value])
        ),
      ...
      <div className="create-appt-card">
        ...
        <form className="create-appt-form" onSubmit={handleSubmit}>
          ...
          <div className="create-appt-grid">
            ...
            <label className="create-appt-field">{fieldLabel("Postcode", true)}{input("postcode","text","Postcode")}</label>
            <label className="create-appt-field">{fieldLabel("Phone number", true)}{input("phone_number_1","tel","Phone number")}</label>
            <label className="create-appt-field">{fieldLabel("Email address", true)}{input("email_address","email","Email address")}</label>
            <label className="create-appt-field">{fieldLabel("Address", true)}{input("address","text","Address")}</label>
            <label className="create-appt-field">{fieldLabel("Measure", true)}{input("product","text","Measure")}</label>
            <label className="create-appt-field">{fieldLabel("Job type", true)}{select("job_type", jobTypes, "Select job type")}</label>
            <label className="create-appt-field">{fieldLabel("Lead source", true)}{select("lead_source", leadSources, "Select lead source")}</label>
            <label className="create-appt-field full">{fieldLabel("Sales note")}<textarea className="create-appt-textarea" value={form.sales_notes} placeholder="Sales note" onChange={(event)=>update("sales_notes",event.target.value)}/></label>
          </div>

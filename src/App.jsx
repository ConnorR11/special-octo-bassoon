...
function App() {
  ...
  function handleAppointmentSelect(appointment) {
    ...
    setSelected(null); setPickupAppointment(null); setSelectedAppointment(mappedAppointment); setPage("appointments")
  ...
  function handleSignOut() {
    if (supabase) supabase.auth.signOut().catch((err) => console.error("Error signing out:", err))
  }
        ...
        {pickupAppointment ? (
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", padding: "10px 24px 0", background: "#fff" }}>
              <AppointmentActions appointment={selectedAppointment} permissionLevel={effectivePermissionLevel} role={effectiveRole} userEmail={effectiveUserEmail} onUpdated={handleAppointmentUpdated} onConfirmLegacy={handleLegacyConfirm} onResultLegacy={handleLegacyResult} onOpenPickup={handleOpenPickup} />
            </div>
            <div className="appointment-detail-host"><AppointmentDetail appointment={selectedAppointment} onBack={handleBackToAppointments} onUpdated={handleAppointmentUpdated} permissionLevel={effectivePermissionLevel} /></div>
          </div>
        ) : selected ? (
          ...
          <FitSheet contracts={allDeals} loading={reportingLoading} setSelected={setSelected} onSelectDeal={setSelected} />
        ) : page === "epvs" ? (
          <EPVSCalculator />
        ) : (
          <Dashboard contracts={allDeals} total={totalValue} avg={averageValue} upcoming={upcomingInstallations} setSelected={setSelected} />

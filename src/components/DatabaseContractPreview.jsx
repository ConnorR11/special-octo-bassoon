...
function ContractPage({ page, appointment, number }) {
  ...
  const isSystemOverview = settings.page_kind === "system_overview" || number === 2
  const openSolarImage = appointment?.open_solar_image || appointment?.openSolar?.systemImageUrl || appointment?.openSolar?.imageUrl || ""
  ...
  return (
    <div style={{ width: "210mm", minHeight: "297mm", background, color: textColor, boxSizing: "border-box", padding, position: "relative", boxShadow: "0 20px 60px rgba(0,0,0,.35)", flex: "0 0 auto" }}>
      ...
      {isSystemOverview && openSolarImage && <div style={{ marginTop: 22, marginBottom: 24, borderRadius: 10, overflow: "hidden", border: "1px solid #dfe5e9", background: "#f5f7f9" }}><img src={openSolarImage} alt="OpenSolar system design" style={{ display: "block", width: "100%", height: "auto", maxHeight: 245, objectFit: "contain" }} /></div>}
      {page?.body && <div style={{ marginTop: isSystemOverview && openSolarImage ? 0 : 26, whiteSpace: "pre-wrap", fontSize: 11, lineHeight: 1.65, color: visual.body_color || "#52606d" }}>{interpolate(page.body, appointment)}</div>}
...
export default function DatabaseContractPreview({ appointment, templateId, onClose }) {
  ...
  const [previewAppointment, setPreviewAppointment] = useState(appointment || { name: "Presentation Preview", product: "Solar" })
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      let nextAppointment = appointment || { name: "Presentation Preview", product: "Solar" }
      if (!nextAppointment.open_solar_image) {
        const { data: sampleAppointment } = await supabase.from("appointments").select("name,product,open_solar_image").ilike("product", "%solar%").not("open_solar_image", "is", null).order("appointment_updated_time", { ascending: false, nullsFirst: false }).limit(1).maybeSingle()
        if (sampleAppointment?.open_solar_image) nextAppointment = { ...nextAppointment, open_solar_image: sampleAppointment.open_solar_image }
      }

      if (mounted) { setTemplate(templateData); setPages(pageData || []); setPreviewAppointment(nextAppointment); setIndex(0); setLoading(false) }
    }
    load()
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 4000, background: "#111820", color: "#fff", display: "flex", flexDirection: "column" }}>
      <div style={{ height: 58, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 22px", borderBottom: "1px solid rgba(255,255,255,.12)", flex: "0 0 auto" }}>
        <div><div style={{ fontSize: 14, fontWeight: 800 }}>{template?.name || "Digital Solar Contract"}</div><div style={{ fontSize: 10, opacity: .6, marginTop: 3 }}>{previewAppointment?.name || "Presentation Preview"}</div></div>

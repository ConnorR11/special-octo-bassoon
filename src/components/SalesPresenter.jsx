...
// CUSTOMER_NAME_FIX: resolve the presenter customer directly from appointment.name.
...
function SolarCoverSlide({ appointment }) {
  const customer = String(appointment?.name || "").trim() || String(appointment?.customer_name || "").trim() || "Your Home"
  const benefits = [[Sun, "Generate"], [BatteryCharging, "Store"], [PoundSterling, "Save"]]
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", minHeight: 0, overflow: "hidden", display: "flex", color: "#fff", background: "radial-gradient(circle at 70% 38%, #12639a 0%, #083f69 35%, #032d50 68%, #021f38 100%)" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, width: "58%", padding: "clamp(30px,5vw,68px) clamp(28px,5vw,70px) 38px", display: "flex", flexDirection: "column", justifyContent: "space-between", boxSizing: "border-box" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div><div style={{ fontSize: "clamp(10px,1vw,14px)", fontWeight: 800, letterSpacing: ".08em" }}>HOMESHIELD SCOTLAND LTD</div><div style={{ marginTop: 4, fontSize: "clamp(8px,.8vw,11px)", color: "rgba(255,255,255,.66)" }}>Enhancing Homes Across Scotland</div></div>
          </div>
          <div style={{ marginTop: "clamp(45px,8vh,82px)" }}>
            <div style={{ display: "inline-block", padding: "7px 11px", borderRadius: 999, background: "rgba(125,211,252,.10)", border: "1px solid rgba(125,211,252,.25)", color: "#7dd3fc", fontSize: "clamp(9px,.9vw,12px)", fontWeight: 800, letterSpacing: ".15em", textTransform: "uppercase", marginBottom: 17 }}>Your Solar Journey</div>
            <div style={{ fontSize: "clamp(38px,5.1vw,74px)", lineHeight: .96, fontWeight: 800, letterSpacing: "-.045em" }}>Solar Energy<br />Systems</div>
            <div style={{ marginTop: 20, maxWidth: 560, fontSize: "clamp(14px,1.45vw,21px)", lineHeight: 1.4, color: "rgba(255,255,255,.78)" }}>Designed around your home.<br />Built for your energy future.</div>
          </div>
        </div>
      ...
      <div style={{ position: "relative", zIndex: 1, flex: 1, minWidth: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: "35px 32px 70px 0", boxSizing: "border-box" }}><div style={{ position: "relative", width: "min(410px,40vw)", aspectRatio: "1.05" }}><div style={{ position: "absolute", top: "2%", right: "6%", width: "24%", aspectRatio: 1, borderRadius: "50%", background: "radial-gradient(circle, rgba(255,221,130,.95), rgba(255,184,70,.22) 42%, transparent 72%)" }} /><div style={{ position: "absolute", left: "10%", right: "3%", bottom: "8%", height: "57%", background: "linear-gradient(135deg,#edf6f8,#b7d0da)", clipPath: "polygon(7% 35%,50% 0,93% 35%,93% 100%,7% 100%)", boxShadow: "0 25px 55px rgba(0,0,0,.32)" }} /><div style={{ position: "absolute", left: "3%", right: "10%", top: "27%", height: "31%", background: "linear-gradient(180deg,#123f62,#071e35)", clipPath: "polygon(0 100%,50% 0,100% 100%)" }} /><div style={{ position: "absolute", left: "17%", right: "20%", top: "30%", height: "22%", transform: "skewX(-10deg)", background: "repeating-linear-gradient(90deg,#123f61 0 17px,#5e91ae 18px 19px),repeating-linear-gradient(0deg,transparent 0 13px,rgba(255,255,255,.35) 14px 15px)", border: "2px solid rgba(255,255,255,.3)", boxShadow: "0 8px 25px rgba(0,0,0,.25)" }} /><div style={{ position: "absolute", left: "42%", bottom: "8%", width: "14%", height: "30%", background: "#7893a0", borderRadius: "4px 4px 0 0" }} /><div style={{ position: "absolute", left: "16%", bottom: "8%", width: "19%", height: "20%", background: "#183d55", border: "3px solid rgba(255,255,255,.18)" }} /><div style={{ position: "absolute", right: "7%", bottom: "7%", width: 66, height: 100, borderRadius: 9, background: "linear-gradient(145deg,#f5fbfd,#b9d6e0)", boxShadow: "0 18px 35px rgba(0,0,0,.25)", border: "1px solid rgba(255,255,255,.45)" }}><div style={{ margin: 9, height: 5, borderRadius: 5, background: "#2499ed" }} /><div style={{ margin: "15px 12px", height: 40, borderRadius: 5, background: "#d7e7ed", border: "1px solid #a9c3ce" }} /></div><div style={{ position: "absolute", left: "-5%", right: "-5%", bottom: 0, height: 35, borderRadius: "50%", background: "rgba(2,20,35,.55)", filter: "blur(12px)" }} /></div></div>
      <div style={{ position: "absolute", zIndex: 4, left: "50%", bottom: 20, transform: "translateX(-50%)", display: "flex", gap: 8, whiteSpace: "nowrap" }}>{benefits.map(([Icon, label]) => <div key={label} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 8, background: "rgba(0,0,0,.16)", border: "1px solid rgba(255,255,255,.1)", fontSize: "clamp(8px,.75vw,10px)", color: "rgba(255,255,255,.72)" }}><Icon size={12} />{label}</div>)}</div>
    </div>
  )
}

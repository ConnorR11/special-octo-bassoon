...
function SolarCoverSlide({ appointment }) {
  const customer = appointment?.name || appointment?.customer_name || "Your Home"
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", minHeight: 0, overflow: "hidden", display: "flex", color: "#fff", background: "radial-gradient(circle at 70% 38%, #12639a 0%, #083f69 35%, #032d50 68%, #021f38 100%)" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, width: "58%", padding: "clamp(30px,5vw,68px) clamp(28px,5vw,70px) 38px", display: "flex", flexDirection: "column", justifyContent: "space-between", boxSizing: "border-box" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: "clamp(10px,1vw,14px)", fontWeight: 800, letterSpacing: ".08em" }}>HOMESHIELD SCOTLAND LTD</div>
              <div style={{ marginTop: 4, fontSize: "clamp(8px,.8vw,11px)", color: "rgba(255,255,255,.66)" }}>Enhancing Homes Across Scotland</div>
            </div>
          </div>
      ...
      <div style={{ position: "absolute", zIndex: 4, left: "50%", bottom: 20, transform: "translateX(-50%)", display: "flex", gap: 8, whiteSpace: "nowrap" }}>
        {benefits.map(([Icon, label]) => <div key={label} style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 8, background: "rgba(0,0,0,.16)", border: "1px solid rgba(255,255,255,.1)", fontSize: "clamp(8px,.75vw,10px)", color: "rgba(255,255,255,.72)" }}><Icon size={12} />{label}</div>)}
      </div>
    </div>
  )
}

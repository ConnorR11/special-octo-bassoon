...
function NavItem({ icon: Icon, label, active = false, onClick, disabled = false, locked = false }) { return <button type="button" className={`sidebar-subitem ${active ? "active" : ""} ${disabled ? "disabled" : ""}`} onClick={disabled ? undefined : onClick} disabled={disabled}><span className="sidebar-subitem-icon"><Icon size={15} /></span><span>{label}</span>{locked ? <Lock size={13} color="#b8c0c8" style={{ marginLeft: "auto" }} /> : disabled ? <span className="coming-soon">Soon</span> : null}</button> }

  ...
  return <>
    ...
    <aside className={`sidebar ${mobile ? "sidebar-open" : ""}`}>
      ...
      <nav className="sidebar-nav">
        ...
        <Folder title="Procurement" icon={Wrench} open={openFolders.procurement} onClick={() => toggleFolder("procurement")}><NavItem icon={ClipboardCheck} label="Surveys" disabled /><NavItem icon={CalendarDays} label="Cover Calls" disabled /><NavItem icon={AlertTriangle} label="Costing" disabled /><NavItem icon={AlertTriangle} label="Ordering" disabled /></Folder>
        <Folder title="Installations" icon={Wrench} open={openFolders.installation} onClick={() => toggleFolder("installation")}><NavItem icon={ClipboardCheck} label="Fit Sheet" active={isActive("fitsheet")} onClick={() => navigate("fitsheet")} /><NavItem icon={CalendarDays} label="Installations" disabled /><NavItem icon={AlertTriangle} label="Installation Issues" disabled /></Folder>
        <Folder title="Finance" icon={PoundSterling} open={openFolders.finance} onClick={() => toggleFolder("finance")}><NavItem icon={PoundSterling} label="Revenue" disabled /><NavItem icon={CreditCard} label="Payments" disabled /><NavItem icon={Receipt} label="Invoices" disabled /></Folder>
...
function NavItem({ icon: Icon, label, active = false, onClick, disabled = false }) { return <button type="button" className={`sidebar-subitem ${active ? "active" : ""} ${disabled ? "disabled" : ""}`} onClick={disabled ? undefined : onClick} disabled={disabled}><span className="sidebar-subitem-icon"><Icon size={15} /></span><span>{label}</span>{disabled && <span className="coming-soon">Soon</span>}</button> }

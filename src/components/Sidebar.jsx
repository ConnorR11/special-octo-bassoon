  ...
  return <>
    ...
    <aside className={`sidebar ${mobile ? "sidebar-open" : ""}`}>
      ...
      <nav className="sidebar-nav">
        ...
        <Folder title="Procurement" icon={Wrench} open={openFolders.procurement} onClick={() => toggleFolder("procurement")}>
          <NavItem icon={ClipboardCheck} label="Surveys" disabled /><NavItem icon={CalendarDays} label="Cover Calls" disabled /><NavItem icon={AlertTriangle} label="Costing" disabled /><NavItem icon={AlertTriangle} label="Ordering" disabled />
        </Folder>
        <Folder title="Installations" icon={Wrench} open={openFolders.installation} onClick={() => toggleFolder("installation")}>
          <NavItem icon={ClipboardCheck} label="Fit Sheet" active={isActive("fitsheet")} onClick={() => navigate("fitsheet")} /><NavItem icon={KanbanSquare} label="Kanban" active={isActive("installations")} onClick={() => navigate("installations")} /><NavItem icon={AlertTriangle} label="Installation Issues" disabled />
        </Folder>
        <Folder title="Remedials" icon={Headphones} open={openFolders.customerService} onClick={() => toggleFolder("customerService")}>
          <NavItem icon={UserRound} label="Customers" disabled /><NavItem icon={MessageCircle} label="Follow-ups" disabled /><NavItem icon={AlertTriangle} label="Complaints" disabled />
        </Folder>
        <Folder title="Accounts" icon={PoundSterling} open={openFolders.finance} onClick={() => toggleFolder("finance")}>
          <NavItem icon={PoundSterling} label="Revenue" disabled /><NavItem icon={CreditCard} label="Payments" disabled /><NavItem icon={Receipt} label="Invoices" disabled />
        </Folder>
        <Folder title="Documents" icon={Files} open={openFolders.documents} onClick={() => toggleFolder("documents")}>
          <NavItem icon={FileText} label="Company Brochures" disabled /><NavItem icon={Files} label="Customer Documents" disabled /><NavItem icon={FilePlus} label="Templates" active={isActive("templates")} onClick={() => navigate("templates")} />
        </Folder>
        {isAdministrator && <Folder title="Administration" icon={Settings} open={openFolders.admin} onClick={() => toggleFolder("admin")}>
          ...
          <NavItem icon={Presentation} label="Sales Presentations" active={isActive("sales-presentations")} onClick={() => navigate("sales-presentations")} />
...
function Folder({ title, icon: Icon, open, onClick, children }) {
  return <div className="sidebar-folder"><button type="button" className="sidebar-folder-header" onClick={onClick}><span className="sidebar-folder-left"><Icon size={17} /><span>{title}</span></span>{open ? <ChevronDown size={15} /> : <ChevronRight size={15} />}</button>{open && <div className="sidebar-folder-items">{children}</div>}</div>
}

function NavItem({ icon: Icon, label, active = false, onClick, disabled = false, locked = false }) {
  return <button type="button" className={`sidebar-subitem ${active ? "active" : ""} ${disabled ? "disabled" : ""}`} onClick={disabled ? undefined : onClick} disabled={disabled}><span className="sidebar-subitem-icon"><Icon size={15} /></span><span>{label}</span>{locked ? <Lock size={13} color="#b8c0c8" style={{ marginLeft: "auto" }} /> : disabled ? <span className="coming-soon">Soon</span> : null}</button>
}

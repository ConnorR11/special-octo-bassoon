...
export default function ThirtyYearBreakdown({ thirtyYearProjection }) {
  ...
  const selectedScenario =
    scenarioList.find((scenario) => scenario.key === selectedKey) || scenarioList[0]
  ...
  const totals = useMemo(
    () =>
      displayRows.reduce(
        (total, row) => ({
          generation: total.generation + valueFrom(row, "generation"),
          solarBenefit: total.solarBenefit + row.solarBenefit,
          batterySelfConsumptionBenefit:
            total.batterySelfConsumptionBenefit +
          ...
          billPostInstall:
            total.billPostInstall + valueFrom(row, "billPostInstall"),
        }),
        {
          generation: 0,
          solarBenefit: 0,
          batterySelfConsumptionBenefit: 0,
  ...
  const firstYear = displayRows[0]
  const firstYearBenefit = firstYear
    ? firstYear.solarBenefit +
      firstYear.batterySelfConsumptionBenefit +
      firstYear.forceChargeBenefit +
      firstYear.exportBenefit
    : 0
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div className="card" style={{ padding: 20, marginBottom: 14 }}>
        <h2 style={{ margin: 0, fontSize: 20, color: "#475569" }}>
        ...
        >
          The estimated savings below are based on the customer's annual
          electricity consumption and the assumptions entered into the EPVS
          calculator. They are provided for illustration and are not a
          guarantee of performance. Replacement, maintenance and cleaning
          costs are not currently included in this preliminary model.
        </p>
      </div>
          ...
          {scenarioList.map((item) => {
            ...
            return (
              <button
                ...
                style={{
                  border: active ? "1px solid #299d48" : "1px solid #d7dee8",
      ...
      >
        ...
        <SummaryCard
          ...
          value={
            paybackRow
              ? `${paybackRow.year} years`
              : "Not achieved"
          }
        />
        <SummaryCard
          label="Total net savings"
          value={money(finalNetPosition)}
        />
        <SummaryCard
          ...
          >
            <thead>
              <tr>
                ...
                <HeaderCell minWidth={92}>NET ANNUAL<br />BENEFIT</HeaderCell>
                <HeaderCell green minWidth={88}>NET<br />POSITION</HeaderCell>
                <HeaderCell>BILL PRE<br />INSTALL</HeaderCell>
                <HeaderCell>BILL POST<br />INSTALL</HeaderCell>
              </tr>
            </thead>
            ...
            <tbody>
              ...
              {displayRows.length > 0 && (
                <tr>
                  <td style={{ ...totalCell, textAlign: "left" }}>TOTALS</td>
                  ...
                  <td style={totalCell}>{money(totals.yearlyPayment)}</td>
                  <td style={totalCell}>{money(totals.netAnnualBenefit)}</td>
                  <td style={{ ...totalCell, background: "#299d48" }}>
                    {money(finalNetPosition)}
                  </td>
                  <td style={totalCell}>{money(totals.billPreInstall)}</td>
                  <td style={totalCell}>{money(totals.billPostInstall)}</td>
                </tr>
              )}
            </tbody>
          </table>

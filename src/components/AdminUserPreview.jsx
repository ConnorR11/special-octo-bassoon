...
export default function AdminUserPreview({ activeUser, onStart, onStop }) {
  ...
  return <div style={{ position: "fixed", right: 24, top: 74, zIndex: 900, width: 260, background: "#fff", border: "1px solid #dfe3e8", borderRadius: 9, boxShadow: "0 8px 28px rgba(0,0,0,.12)" }}><button type="button" onClick={() => setOpen((value) => !value)} style={{ width: "100%", height: 40, border: 0, borderRadius: 9, background: "#fff", display: "flex", alignItems: "center", gap: 8, padding: "0 12px", cursor: "pointer", fontSize: 11, fontWeight: 700, color: "#172033" }}><Eye size={15} /> View CRM as user</button>{open && <div style={{ padding: "10px 12px", borderTop: "1px solid #eee" }}><select value={selectedId} onChange={(e) => setSelectedId(e.target.value)} style={{ width: "100%", height: 34, border: "1px solid #d9dadd", borderRadius: 6, padding: "0 8px", fontSize: 11 }}><option value="">Select user...</option>{users.map((user) => <option key={user.id} value={user.id}>{user.full_name || user.email} · {user.role || "User"}</option>)}</select><button type="button" disabled={!selectedId} onClick={() => { const user = users.find((item) => String(item.id) === selectedId); if (user) { onStart(user); setOpen(false) } }} style={{ width: "100%", marginTop: 8, height: 34, border: 0, borderRadius: 6, background: selectedId ? "#172554" : "#cbd5e1", color: "#fff", cursor: selectedId ? "pointer" : "default", fontSize: 11, fontWeight: 700 }}>Start preview</button></div>}</div>
}

...
function header(pdf, settings) {
  ...
  if (settings.show_header !== false) { pdf.setFillColor(...accent); pdf.rect(0, 0, width, 14, "F"); pdf.setTextColor(255,255,255); pdf.setFont("helvetica","bold"); pdf.setFontSize(8); pdf.text("HOMESHIELD SCOTLAND LTD",padding,9); pdf.setFont("helvetica","normal"); pdf.setFontSize(7); pdf.text(CONTRACT_NAME,width-padding,9,{align:"right"}) }
  return { width, height, accent, text, padding, y: settings.show_header === false ? padding : 24 }
}

function title(pdf, page, ctx) {
  const terms = page?.settings?.page_kind === "terms_conditions"; const titleY = terms ? ctx.y - 2 : ctx.y + 4
  pdf.setTextColor(...ctx.text); pdf.setFont("helvetica","bold"); pdf.setFontSize(terms ? 16 : 22); pdf.text(page.title || "",ctx.padding,titleY)
  if (page.subtitle) { pdf.setFont("helvetica","normal"); pdf.setFontSize(terms ? 7.5 : 9); pdf.setTextColor(100,112,120); if (terms) pdf.text(page.subtitle,ctx.width-ctx.padding,titleY,{align:"right"}); else pdf.text(page.subtitle,ctx.padding,ctx.y+11) }
  pdf.setFillColor(...ctx.accent); if (terms) pdf.rect(ctx.padding,ctx.y+2,ctx.width-(ctx.padding*2),1.2,"F"); else pdf.rect(ctx.padding,ctx.y+15,28,1.2,"F")
}
...
function body(pdf, content, x, y, width, textRgb, appointment, epvs) { if(!content)return y; const withoutImageToken=String(content).replace(/(^|\r?\n)\s*{{open_solar_image}}\s*(?=\r?\n|$)/g,"$1");pdf.setFont("helvetica","normal");pdf.setFontSize(9);pdf.setTextColor(...textRgb);interpolate(withoutImageToken,appointment,epvs).split(/\r?\n/).forEach(line=>{if(!line.trim()){y+=4;return}pdf.splitTextToSize(line,width).forEach(part=>{pdf.text(part,x,y);y+=4.8});y+=2});return y }

function drawItemisedBreakdown(pdf,page,ctx,data,results,appointment,epvs){ const width=ctx.width-ctx.padding*2; const settings=page.settings||{}; const configured=Array.isArray(settings.included_items)?settings.included_items:[]; const items=configured.map(item=>typeof item==="string"?{name:item,quantity:1,type:""}:{name:item?.name??"—",quantity:item?.quantity??1,type:item?.type??""}); const headerY=ctx.y+28; const typeX=ctx.padding+width-43; const rowHeight=7.15; pdf.setFillColor(...ctx.accent);pdf.roundedRect(ctx.padding,headerY-7,width,11,2,2,"F");pdf.setTextColor(255,255,255);pdf.setFont("helvetica","bold");pdf.setFontSize(8);pdf.text("PRODUCT / SERVICE",ctx.padding+7,headerY);pdf.text("TYPE",typeX,headerY,{align:"center"});pdf.text("QTY",ctx.padding+width-7,headerY,{align:"right"});let y=headerY+9;items.forEach((item,index)=>{const name=interpolate(String(item.name),appointment,epvs);const type=interpolate(String(item.type),appointment,epvs);const quantity=interpolate(String(item.quantity),appointment,epvs);if(index%2===0){pdf.setFillColor(247,249,250);pdf.roundedRect(ctx.padding,y-5.2,width,rowHeight,1.2,1.2,"F")}pdf.setTextColor(...ctx.text);pdf.setFont("helvetica","normal");pdf.setFontSize(8.1);pdf.text(name,ctx.padding+7,y);if(type){pdf.setFont("helvetica","bold");pdf.setFontSize(6.5);const tagWidth=pdf.getTextWidth(type)+6;const typeKey=type.toLowerCase();const fill=typeKey==="service"?[255,241,230]:typeKey==="product"?[231,242,248]:[238,240,242];const colour=typeKey==="service"?[199,106,0]:typeKey==="product"?[11,93,138]:[75,85,92];pdf.setFillColor(...fill);pdf.setTextColor(...colour);pdf.roundedRect(typeX-tagWidth/2,y-3.8,tagWidth,4.5,2,2,"F");pdf.text(type,typeX,y-0.5,{align:"center"})}pdf.setTextColor(...ctx.text);pdf.setFont("helvetica","bold");pdf.setFontSize(8.1);pdf.text(quantity,ctx.padding+width-7,y,{align:"right"});y+=rowHeight});y+=7;const price=results?.systemCost??data?.systemCost??appointment?.system_cost??appointment?.contract_value??appointment?.sale_value??appointment?.price;pdf.setFillColor(...ctx.accent);pdf.roundedRect(ctx.padding,y,width,25,3,3,"F");pdf.setTextColor(255,255,255);pdf.setFont("helvetica","normal");pdf.setFontSize(8);pdf.text("TOTAL SYSTEM PRICE",ctx.padding+8,y+10);pdf.setFont("helvetica","bold");pdf.setFontSize(18);pdf.text(money(price),ctx.padding+width-8,y+15,{align:"right"}) }

function parseTermsSections(raw){const sections=[];let current=[];String(raw||"").replace(/\r/g,"").split("\n").forEach(line=>{const trimmed=line.trim();if(/^\d+\.\s+/.test(trimmed)){if(current.length)sections.push(current.join(" ").trim());current=[trimmed]}else if(trimmed)current.push(trimmed)});if(current.length)sections.push(current.join(" ").trim());return sections.filter(Boolean)}
...
export async function GenerateSolarContract({ appointment, epvsCalculation }) { if(!appointment)return; const {data:template,error:templateError}=await supabase.from("templates").select("id,name,template_type,active").eq("name",CONTRACT_NAME).eq("active",true).maybeSingle(); if(templateError)throw templateError; if(!template)throw new Error(`Active ${CONTRACT_NAME} template could not be found.`); const {data:pages,error:pagesError}=await supabase.from("template_pages").select("id,title,subtitle,body,settings,slide_order").eq("presentation_id",template.id).order("slide_order",{ascending:true}); if(pagesError)throw pagesError; if(!pages?.length)throw new Error("The Digital Solar Contract template has no pages configured."); let epvs=epvsCalculation||appointment?.epvs_calculation||null; if(typeof epvs==="string"){try{epvs=JSON.parse(epvs)}catch{}} const pageSize=pages.find(p=>p.settings?.page_size)?.settings?.page_size||"A4"; const orientation=pages.find(p=>p.settings?.orientation)?.settings?.orientation||"portrait"; const pdf=new jsPDF({unit:"mm",format:pageSize.toLowerCase(),orientation}); for(let i=0;i<pages.length;i+=1){if(i>0)pdf.addPage(pageSize.toLowerCase(),orientation);const page=pages[i];await renderPage(pdf,page,i,pages.length,appointment,epvs);footer(pdf,i,pages.length,page.settings||{},appointment)} const safeName=textValue(appointment?.name,"Customer").replace(/[^a-z0-9]+/gi,"-").replace(/^-+|-+$/g,"")||"Customer"; pdf.save(`${safeName}-Digital-Solar-Contract.pdf`) }

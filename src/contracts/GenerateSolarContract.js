...
function body(pdf, content, x, y, width, textRgb, appointment, epvs) {
  ...
  lines.forEach((line) => {
    ...
    pdf.splitTextToSize(line, width).forEach((part) => { pdf.text(part, x, y); y += 4.8 })
    y += 2
  })
  return y
}
...
async function renderPage(pdf, page, appointment, epvs) {
  ...
  // Any template page can opt into the OpenSolar image simply by putting
  // {{open_solar_image}} in its body. This is deliberately handled here,
  // before the page-kind renderer, so page 2/itemised_breakdown works too.
  if (String(page.body || "").includes("{{open_solar_image}}")) {
    y = await drawImage(pdf, appointment?.open_solar_image, ctx.padding, y, width, 72)
  }
...
export async function GenerateSolarContract({ appointment, epvsCalculation }) {
  ...
  for (let i = 0; i < pages.length; i += 1) {
    ...
    await renderPage(pdf, page, appointment, epvsCalculation)
  }

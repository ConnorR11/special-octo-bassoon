...
function getOpenSolarImageUrl(appointment, epvs) {
  ...
  return String(appointment?.open_solar_image || openSolar.systemImageUrl || openSolar.imageUrl || appointment?.openSolarImageUrl || "").trim()
}
...
function drawItemisedBreakdown(pdf, page, ctx, appointment, epvs, startY = ctx.y) {
  ...
  const configuredItems = Array.isArray(page.settings?.included_items) ? page.settings.included_items : []
  const items = configuredItems.map((item) => typeof item === "string" ? { name: item, quantity: 1, type: "" } : { name: item?.name ?? "—", quantity: item?.quantity ?? 1, type: item?.type ?? "" })
  ...
  items.forEach((item, index) => {
    ...
    if (itemType) {
      ...
      const tagY = y - 3.8
      if (typeText.toLowerCase() === "service") {
        pdf.setFillColor(255, 241, 230)
        pdf.setTextColor(199, 106, 0)
      } else if (typeText.toLowerCase() === "product") {
        pdf.setFillColor(231, 242, 248)
        pdf.setTextColor(11, 93, 138)
      } else {
...
async function renderPage(pdf, page, appointment, epvs) {
  ...
  if (kind === "system_overview") {
    ...
    if (openSolarImageUrl) {
      y = await drawImage(pdf, openSolarImageUrl, ctx.padding, y, width, 72)
    }
    y = rows(pdf, [
...
export async function GenerateSolarContract({ appointment, epvsCalculation }) {
  ...
  if (appointment?.appointment_row_id) {
    ...
    if (appointmentError) {
      ...
      contractAppointment = { ...appointment, open_solar_image: appointmentRow.open_solar_image }
    }
  }
  ...
  for (let i = 0; i < pages.length; i += 1) {
    ...
    await renderPage(pdf, page, contractAppointment, epvsCalculation)
  }

  pages.forEach((page, i) => {
    pdf.setPage(i + 1)
    footer(pdf, i, pages.length, page.settings || {}, contractAppointment)
  })

  const safeName = String(contractAppointment.name || "Customer").replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "") || "Customer"

...
function drawItemisedBreakdown(pdf, page, ctx, appointment, epvs) {
  ...
  const items = configuredItems.map((item) => {
    if (typeof item === "string") {
      return { name: item, quantity: 1, type: "" }
    }
  ...
  items.forEach((item, index) => {
    ...
    if (itemType) {
      ...
      pdf.roundedRect(tagX, tagY, tagWidth, tagHeight, 2, 2, "F")
  ...
  pdf.setFillColor(...ctx.accent)
  pdf.roundedRect(ctx.padding, y, width, boxHeight, 3, 3, "F")

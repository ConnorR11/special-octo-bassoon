export default async function handler(req, res) {
  ...
  const findActivation = (part, activations, kind) => {
    ...
    const candidateIds = [part?.[`${kind}_activation_id`], part?.activation_id, partData?.[`${kind}_activation_id`], partData?.activation_id, part?.activation?.id]
      .map((v) => String(v || "").trim()).filter(Boolean)
    for (const id of candidateIds) {
      const direct = activations.find((item) => String(item?.id || "") === id)
      if (direct) return direct
    }
    const codeCandidates = [part?.code, partData?.code, part?.model, partData?.model, part?.sku, partData?.sku].map(normalise).filter(Boolean)
  ...
  try {
    ...
    const inverterParts = systemInverters.map((part) => {
      ...
      // Use OpenSolar max_power_kw as the authoritative inverter capacity.
    ...
    const batteryParts = systemBatteries.map((part) => {
      ...
      return { manufacturer: String(part?.manufacturer_name || activation?.manufacturer_name || activationData?.manufacturer_name || ""), model: String(part?.code || activation?.code || activationData?.code || ""), quantity: Math.max(1, Number(part?.quantity || 1)), capacityKwh: capacity || 0, roundTripEfficiencyPercent: efficiency == null ? null : efficiency <= 1 ? efficiency * 100 : efficiency, depthOfDischargePercent: dod == null ? null : dod <= 1 ? dod * 100 : dod, endOfLifeCapacityPercent: endOfLifeCapacity == null ? null : endOfLifeCapacity <= 1 ? endOfLifeCapacity * 100 : endOfLifeCapacity, warrantyYears }
    })
    ...
    const weighted = (parts, valueKey, capacityKey) => { let numerator = 0; let denominator = 0; parts.forEach((part) => { const value = Number(part?.[valueKey]); const weight = Number(part?.[capacityKey] || 0) * Math.max(1, Number(part?.quantity || 1)); if (Number.isFinite(value) && weight > 0) { numerator += value * weight; denominator += weight } }); return denominator > 0 ? numerator / denominator : null }

    const arrays = systems.flatMap((system) => {
      const shadeFactor = Number(system?.data?.mcs?.shadingFactor ?? 1)
      const specificYield = String(system?.data?.mcs?.mcsSpecificYieldBeforeShading || "").split(/\n+/).map((line) => { const match = line.match(/:\s*([0-9]+(?:\.[0-9]+)?)/); return match ? Number(match[1]) : null }).filter((value) => Number.isFinite(value))
      return (Array.isArray(system?.module_groups) ? system.module_groups : []).map((group, index) => { const azimuth = Number(group?.azimuth ?? 180); const orientation = Math.round(Math.abs(((azimuth - 180 + 540) % 360) - 180)); return { panelCount: Number(group?.module_quantity || 0), orientation, pitch: Math.round(Number(group?.slope || 0)), shading: Number.isFinite(shadeFactor) ? shadeFactor : 1, irradiance: Number(specificYield[index] || 0) } })
    })
    ...
    return res.status(200).json({ success: true, projectId, numberOfArrays: arrays.length, arrays: arrays.slice(0, 3), truncated: arrays.length > 3, systemImageUrl, hardware: { inverter: { manufacturer: inverterParts[0]?.manufacturer || "", model: inverterParts[0]?.model || "", quantity: inverterParts.reduce((total, part) => total + part.quantity, 0), capacityKw: inverterCapacity, efficiencyPercent: weighted(inverterParts, "efficiencyPercent", "capacityKw"), parts: inverterParts }, battery: { manufacturer: batteryParts[0]?.manufacturer || "", model: batteryParts[0]?.model || "", quantity: batteryParts.reduce((total, part) => total + part.quantity, 0), capacityKwh: batteryCapacity, roundTripEfficiencyPercent: weighted(batteryParts, "roundTripEfficiencyPercent", "capacityKwh"), depthOfDischargePercent: weighted(batteryParts, "depthOfDischargePercent", "capacityKwh"), endOfLifeCapacityPercent: weighted(batteryParts, "endOfLifeCapacityPercent", "capacityKwh"), warrantyYears: weighted(batteryParts, "warrantyYears", "capacityKwh"), parts: batteryParts }, evCharger: { manufacturer: evChargerParts[0]?.manufacturer || "", model: evChargerParts[0]?.model || "", quantity: evChargerParts.reduce((total, part) => total + part.quantity, 0), powerKw: evChargerPower, parts: evChargerParts } }, systems: systems.map((system) => ({ id: system?.id, uuid: system?.uuid, name: system?.name, kwStc: system?.kw_stc, totalModuleQuantity: system?.total_module_quantity })) })
  } catch (error) {
